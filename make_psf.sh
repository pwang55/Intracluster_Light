...
usage='
    ...
    In script folder:
    $ ./make_psf.sh path_to_files/cutoutlist.txt (25)
    or
    $ ./make_psf.sh path_to_files/image.fits (25)
    

    In data folder:
    $ path_to_script/make_psf.sh cutoutlist.txt (25)
    or
    $ path_to_script/make_psf.sh image.fits (25)
...
if [ $extension = txt ]; then
    for file in `cat $list_in`; do
        ...
        sex -c ${script_dir}/extra/psf.sex ${path}$file -CATALOG_NAME ${path}${filename}_psf.cat -CATALOG_TYPE FITS_LDAC -PARAMETERS_NAME ${script_dir}/extra/psf.param -FILTER_NAME ${script_dir}/extra/${conv_filter} -WEIGHT_IMAGE ${path}${filename}.wt.fits -VERBOSE_TYPE QUIET
